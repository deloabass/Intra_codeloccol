...
let colonneBoutons = document.querySelectorAll(".buttone");
colonneBoutons.forEach(bouton => {
  bouton.addEventListener("click", function () {
    //Trouver l'élément <tr> parent de ce bouton
    let ligne = bouton.closest('tr');
    //Récupérer le premier élément enfant de la ligne (le premier <td>)
    let premiereCellule = ligne.querySelectorAll('td')[0];
    let valeurPremiereCellule = premiereCellule.textContent;
     //Stocker les valeurs dans le localStorage
    localStorage.setItem("valeurPremiereCellule", JSON.stringify(valeurPremiereCellule));

    // Récupérer le troisième élément enfant de la ligne (le troisième <td>)
    let troisiemeCellule = ligne.querySelectorAll('td')[3];
    let valeurTroisiemeCellule = troisiemeCellule.textContent;
    if (valeurTroisiemeCellule === "Disponible") {
      window.location.href = "/SuiviDetail/index.html";
    } else {
      alert('Le cellule est indisponible maintenant')
    }


    //Pour vérifier que les valeurs ont été stockées, vous pouvez les afficher dans la console
    console.log("La première valeur de la ligne a été stockée dans le localStorage :", valeurPremiereCellule);
  });
});
